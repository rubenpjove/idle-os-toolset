...
- `new_vagrant.sh` now creates the VM, collects MAC/IP, calls `get_os_info.py` (LLM) and automatically executes `update_info_file.sh` with the info obtained from the LLM. This info includes OS_Family, Os_Type and Os_Version.
